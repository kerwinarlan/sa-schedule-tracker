...
The source images live only on the maintainer's machine - they are not
in the repo (confidential). Class blocks are fixed for the semester, so
this is a one-off per semester. Re-run the OCR pass on new images and rebuild the JSON with the
